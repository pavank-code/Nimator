...
class VideoGenerationRequest(BaseModel):
    prompt: str = Field(
        ..., max_length=1000, description="The prompt to generate a video from"
    )
    sample_mode: bool = Field(
        False, description="Generate a 30-second sample instead of full video"
    )
    duration_seconds: int = Field(
        180, description="Target video duration in seconds", ge=30, le=900
    )
...
async def generate_video(
    ...
    request_key = f"{prompt}:{request.sample_mode}:{request.duration_seconds}"
    request_hash = hashlib.md5(request_key.encode()).hexdigest()
    cache_key = f"req_cache:{request_hash}"

    # Check if we already have a job for this exact request
    cached_job_id = redis_client.get(cache_key)
    if cached_job_id:
        # Check if job still exists and is not failed
        job_data_str = redis_client.get(f"job:{cached_job_id}")
        if job_data_str:
            try:
                cached_job = json.loads(job_data_str)
                if cached_job.get("status") != "failed":
                    return VideoGenerationResponse(
                        job_id=cached_job_id,
                        message="Returning existing job for identical request",
    ...
    if len(prompt) < 10:
        raise HTTPException(
            status_code=400, detail="Prompt is too short. Please provide more details."
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate scene plan: {str(e)}"
        )
    ...
    redis_client.expire(f"job:{job_id}", 86400)  # 24 hours TTL

    # Cache the request-to-job mapping
    redis_client.set(cache_key, job_id, ex=86400)

    # Add to render queue
    redis_client.lpush("render_queue", json.dumps(job_data))

    return VideoGenerationResponse(
        job_id=job_id, message="Video generation started successfully", status="queued"
    )
...
async def get_video(job_id: str):
    ...
    if job["status"] != "completed":
        raise HTTPException(
            status_code=400, detail=f"Video not ready. Current status: {job['status']}"
        )
...
async def list_cached_videos():
    ...
    return {"cached_count": len(cached_videos), "videos": cached_videos, "stats": stats}
...
async def check_cache(topic: str):
    ...
    if cached:
        ...
        return {"topic": topic, "cached": True, "video": video_data}
    else:
        return {
            "topic": topic,
            "cached": False,
...
async def clear_topic_cache(topic: str):
    ...
    if deleted > 0:
        return {"status": "deleted", "topic": topic, "entries_deleted": deleted}
    else:
        raise HTTPException(
            status_code=404, detail=f"No cache found for topic: {topic}"
        )
